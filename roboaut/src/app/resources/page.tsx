    ...
    const fetchResources = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/resources?populate[pdf][fields]=url&populate[Photo][fields]=url`
        );
        if (!response.ok) {
              ...
              <div key={resource.id} className="group">
                <div className="glass backdrop-blur-lg rounded-2xl border border-white/10 overflow-hidden hover:border-white/20 transition-all duration-500 hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/20">
                  ...
                  <div className="h-48 bg-gradient-to-br from-gray-800 to-gray-900 relative">
                    ...
                    {/* Type Badge */}
                    <span className="absolute top-4 left-4 px-3 py-1 bg-blue-600 text-white text-sm font-exo2 rounded-lg z-10">
                      {resource.Type}
                    </span>
                  </div>
                    ...
                    <div className="flex gap-2">
                      ...
                      {resource.Links && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 px-4 py-2 bg-gradient-to-r from-blue-600/80 to-purple-600/80 text-white font-exo2 font-medium text-sm rounded-lg transition-all duration-300 hover:from-blue-500 hover:to-purple-500 hover:scale-105 text-center"
                        >
                          Visit Link
                        </a>
                      )}
                      {!resource.pdf?.length && !resource.Links && (
                        <div className="flex-1 px-4 py-2 bg-gray-600/50 text-gray-400 font-exo2 font-medium text-sm rounded-lg text-center cursor-not-allowed">
                          No Downloads
                        </div>
                      )}
                    </div>
